...
abstract class TweetSet extends TweetSetInterface:
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = this.filter(t => !that.contains(t)).filterAcc(t => true, that)
  ...
  def descendingByRetweet: TweetList = 
    ...
    if (max.user != "") new Cons(max, this.remove(max).descendingByRetweet)
    else Nil
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet =     
    TweetReader.allTweets.filter(t => google.exists(k => t.text.contains(k)))

  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter(t => apple.exists(k => t.text.contains(k)))
